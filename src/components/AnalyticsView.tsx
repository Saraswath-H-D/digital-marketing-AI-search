      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="p-5 bg-gradient-to-br from-purple-50 via-white to-indigo-50/50 border border-purple-200/80 rounded-2xl shadow-2xs">
          ...
          <div className="mt-3 flex items-baseline space-x-2">
            <span className="text-2xl font-black text-[var(--text-primary)]">{totalCount.toLocaleString()}</span>
        ...
        <div className="p-5 bg-gradient-to-br from-emerald-50 via-white to-teal-50/50 border border-emerald-200/80 rounded-2xl shadow-2xs">
          ...
          <div className="mt-3 flex items-baseline space-x-2">
            <span className="text-2xl font-black text-[var(--text-primary)]">{verifiedPct}%</span>
            <span className="text-xs font-extrabold text-emerald-600 flex items-center">
        ...
        <div className="p-5 bg-gradient-to-br from-orange-50 via-white to-rose-50/50 border border-orange-200/80 rounded-2xl shadow-2xs">
          ...
          <div className="mt-3 flex items-baseline space-x-2">
            <span className="text-2xl font-black text-[var(--text-primary)]">{highIntentCount.toLocaleString()}</span>
        ...
        <div className="p-5 bg-gradient-to-br from-cyan-50 via-white to-blue-50/50 border border-cyan-200/80 rounded-2xl shadow-2xs">
          ...
          <div className="mt-3 flex items-baseline space-x-2">
            <span className="text-2xl font-black text-[var(--text-primary)]">{enrichedCount.toLocaleString()}</span>
            ...
            {topIndustries.map((ind) => (
              <div key={ind.name} className="space-y-1">
                <div className="flex justify-between text-xs font-semibold">
                  ...
                  <span className="text-[var(--text-muted)] font-mono">{ind.count} ({ind.pct}%)</span>
                </div>
                <div className="w-full h-2.5 bg-[var(--surface-card-header)] rounded-full overflow-hidden">
        ...
        <div className="bg-[var(--surface-card)] p-5 rounded-2xl border border-[var(--border-subtle)] shadow-2xs space-y-4">
          ...
          <div className="space-y-3">
            {seniorityBreakdown.map((s) => (
              <div key={s.level} className="flex items-center justify-between p-3 bg-[var(--surface-card-header)] border border-[var(--border-subtle)] rounded-xl">
                <span className="text-xs font-extrabold text-[var(--text-secondary)]">{s.level}</span>
